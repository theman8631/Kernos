...
class CapabilityRegistry:
        ...
        for cap_name in visible_names:
            ...
            for tool in tool_defs_by_server.get(cap.server_name, []):
                ...
                if name in PRELOADED_TOOLS or name in loaded:
                    continue
                # Build stub description: explicit hint > MCP description > name
                hint = cap.tool_hints.get(name, "")
                if hint:
                    desc = hint
                elif tool.get("description"):
                    # Use the MCP-provided description (first sentence, capped)
                    raw = tool["description"].split(".")[0].strip()
    ...
    def build_tool_directory(self, space: "ContextSpace | None" = None) -> str:
        """Build the capability status section for the system prompt.

        Shows which services are connected and which are available to set up.
        Individual tools are in the tools array (stubs + preloaded + loaded),
        not duplicated here. One source of truth.
        """
        visible_names = self._visible_capability_names(space)
        visible_connected = [
        ...
        if available:
            ...
            for cap in available:
                hint = f' — "{cap.setup_hint}"' if cap.setup_hint else ""
                avail_lines.append(f"  {cap.display_name}: {cap.description}{hint}")
            parts.append(
                "AVAILABLE TO CONNECT (offer to set up if asked):\n"
                + "\n".join(avail_lines)
            )

        parts.append("You cannot do anything beyond your connected services. Be honest about limits.")
        return "\n\n".join(parts)
