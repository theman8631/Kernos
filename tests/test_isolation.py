...
def _make_knowledge(tenant_id: str, entry_id: str, subject: str = "Alice") -> KnowledgeEntry:
    now = _now()
...
async def test_conversation_tenant_a_invisible_to_b(tmp_path):
...
async def test_event_stream_tenant_a_invisible_to_b(tmp_path):
...
async def test_profile_tenant_a_invisible_to_b(tmp_path):
    store = JsonStateStore(tmp_path)
    profile = TenantProfile(tenant_id="tenant_a", status="active", created_at=_now())
    await store.save_tenant_profile("tenant_a", profile)
    assert await store.get_tenant_profile("tenant_b") is None


async def test_profile_each_tenant_sees_own(tmp_path):
    store = JsonStateStore(tmp_path)
    await store.save_tenant_profile("tenant_a", TenantProfile(tenant_id="tenant_a", status="active", created_at=_now()))
    await store.save_tenant_profile("tenant_b", TenantProfile(tenant_id="tenant_b", status="active", created_at=_now()))
    fetched_a = await store.get_tenant_profile("tenant_a")
    fetched_b = await store.get_tenant_profile("tenant_b")
...
async def test_profile_update_a_does_not_affect_b(tmp_path):
    store = JsonStateStore(tmp_path)
    profile_a = TenantProfile(tenant_id="tenant_a", status="active", created_at=_now())
    profile_b = TenantProfile(tenant_id="tenant_b", status="active", created_at=_now())
    await store.save_tenant_profile("tenant_a", profile_a)
    await store.save_tenant_profile("tenant_b", profile_b)
...
async def test_soul_tenant_a_invisible_to_b(tmp_path):
    store = JsonStateStore(tmp_path)
    await store.save_soul(Soul(tenant_id="tenant_a", user_name="Alice", hatched=True))
    assert await store.get_soul("tenant_b") is None


async def test_soul_each_tenant_sees_own(tmp_path):
    store = JsonStateStore(tmp_path)
    await store.save_soul(Soul(tenant_id="tenant_a", user_name="Alice"))
    await store.save_soul(Soul(tenant_id="tenant_b", user_name="Bob"))
...
async def test_soul_update_a_does_not_affect_b(tmp_path):
    store = JsonStateStore(tmp_path)
    soul_a = Soul(tenant_id="tenant_a", user_name="Alice")
    soul_b = Soul(tenant_id="tenant_b", user_name="Bob")
...
async def test_knowledge_tenant_a_invisible_to_b(tmp_path):
    store = JsonStateStore(tmp_path)
...
async def test_knowledge_both_tenants_see_own(tmp_path):
    ...
    assert len(result_a) == 1 and result_a[0].tenant_id == "tenant_a"
    assert len(result_b) == 1 and result_b[0].tenant_id == "tenant_b"
...
async def test_contracts_tenant_a_invisible_to_b(tmp_path):
    store = JsonStateStore(tmp_path)
...
async def test_conversation_summary_tenant_a_invisible_to_b(tmp_path):
    store = JsonStateStore(tmp_path)
    now = _now()
    summary = ConversationSummary(
        tenant_id="tenant_a", conversation_id="conv_1", platform="discord",
...
async def test_conversation_summary_same_conv_id_isolated(tmp_path):
    ...
    await store.save_conversation_summary(ConversationSummary(
        tenant_id="tenant_a", conversation_id="conv_shared", platform="discord",
        message_count=3, first_message_at=now, last_message_at=now,
    ))
    await store.save_conversation_summary(ConversationSummary(
        tenant_id="tenant_b", conversation_id="conv_shared", platform="sms",
...
async def test_tenant_store_each_provisioned_separately(tmp_path):
    store = JsonTenantStore(tmp_path)
    record_a = await store.get_or_create("tenant_a")
    record_b = await store.get_or_create("tenant_b")
    assert record_a["tenant_id"] == "tenant_a"
    assert record_b["tenant_id"] == "tenant_b"


async def test_tenant_store_directories_isolated(tmp_path):
    store = JsonTenantStore(tmp_path)
...
async def test_audit_tenant_b_dir_absent_after_a_logs(tmp_path):
...
async def test_conversation_id_path_separators_stay_in_sandbox(tmp_path):
    """conversation_id with path separators cannot escape the tenant sandbox."""
    store = JsonConversationStore(tmp_path)
