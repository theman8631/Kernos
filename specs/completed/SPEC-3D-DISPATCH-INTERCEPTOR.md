...
if not gate_result.allowed:
    # Return a tool_result that tells the model the action was blocked
    blocked_message = (
    ...
    # This becomes the tool_result in the conversation
    # The model reads it and explains to the user what it wanted to do
```
...
for block in response.content:
    ...
    if effect in ("soft_write", "hard_write", "unknown"):
        ...
        if not gate_result.allowed:
            # Return blocked message as tool_result
            result = (
            ...
            # Skip to appending tool_result, don't execute the tool
            # ... (continue with existing tool_result handling)
            continue
...
3. **_explicit_instruction_matches()** — Step 1 fast path with TOOL_SIGNALS
4. **_check_permission_or_covenant()** — Step 2 with permission lookup + Haiku covenant check
5. **_gate_tool_call()** — orchestrator calling Steps 1-3
6. **_describe_action()** — human-readable action descriptions
7. **Integration into tool-use loop** — insert gate between proposal and execution
8. **permission_overrides on TenantProfile** — field, serialization, backward compat
9. **DISPATCH_GATE EventType** — event emission + GATE: trace logging
10. **Consolidate delete_file** — remove _check_delete_allowed, move signals to TOOL_SIGNALS
11. **Confirmation handling** — recognize confirmations after blocked actions
12. **Tests** — gate classification, fast path, covenant check (authorized/denied/ambiguous), permission override, gate integration in tool loop, blocked message format, confirmation recognition, read bypass, unknown effect handling, delete_file consolidation
13. **Live test**
